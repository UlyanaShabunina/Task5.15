 ...
 */
public class BinaryTree<T extends Comparable<? super T>> implements DefaultBSTree<T> {

    public class SimpleTreeNode implements DefaultBinaryTree.TreeNode<T> {
        public T value;
    ...
    @Override
    public void solve(DefaultBinaryTree<T> analog) {
        JOptionPane.showMessageDialog(null, solve(root)+" ", "Результат", JOptionPane.PLAIN_MESSAGE);
    }
    ...
    public int solve(SimpleTreeNode node) {
        goDown(node, null);
        return global_max_value;
    }

    public int goDown(SimpleTreeNode node, Integer num) {
        if (node==null)return 0;
        int res;
        if(num==null ||num != node.value){
            res = goDown(node, (Integer) node.value);
            if(res > global_max){
                global_max = res;
                global_max_value = (Integer)node.value;
            }
            res = 0;
        } else res = 1;
        return goDown(node.right, num)+goDown(node.left, num)+res;
    }
